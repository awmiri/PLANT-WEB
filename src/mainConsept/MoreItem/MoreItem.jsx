...
export default function MoreItem({ data }) {
    ...
    return (
        <div className='flex items-center justify-between gap-6'>
            {
                data.map((item) => (
                    <div className='w-[280px] h-[240px] relative rounded-[8px] overflow-hidden'>
                        <img src={item.img[0]} alt="" className='w-[280px] h-[240px]' />
                        <div className='bg-black/50 absolute z-10 w-full h-full top-0'></div>
                        <div className='absolute top-[150px] right-5 z-50'>
                            <h3 className='text-white font-vazirBold'>{item.name}</h3>
                            <Link to={`iteminfo/${item.categoriEn}/${item.id}`} className='relative z-10 group overflow-hidden flex items-center justify-center gap-1 mt-1.5 border border-white px-2 py-1.5 rounded-lg'>
                                <span className='flex items-center text-white group-hover:text-black font-vazirMediom text-sm transition duration-200'>
                                    <span>خرید</span>
                                    <span>{item.name}</span>
                                </span>
